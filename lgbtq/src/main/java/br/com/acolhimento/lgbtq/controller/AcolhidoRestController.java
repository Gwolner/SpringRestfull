...
public class AcolhidoRestController {
	...
	public ResponseEntity<?> inserir(@RequestBody Acolhido acolhido) throws Exception {

		int idCoordenada;

		try {
			acolhido.setSenha(Criptografia.gerarHash(acolhido.getSenha()));
	...
	@PostMapping("/Acolhido/Auth")
	public ResponseEntity<String> autenticarAcolhido(@RequestBody Acolhido acolhido) throws Exception {

		String cpfAcolhido = "0";

		try {
			acolhido.setSenha(Criptografia.gerarHash(acolhido.getSenha()));

			cpfAcolhido = Fachada.getCurrentInstance().autenticacaoAcolhido(acolhido.getEmail(), acolhido.getSenha());

			if (cpfAcolhido != "0") {
				return new ResponseEntity<String>(cpfAcolhido, HttpStatus.OK);
			} else {
